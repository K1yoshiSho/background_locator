...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> _updateNotificationText(LocationDto? data) async {
    ...
    await BackgroundLocator.updateNotificationText(
      title: "Workspace",
      msg: "Actual for ${DateFormat().add_Hm().format(DateTime.now())}",
  ...
  Future<bool> _checkLocationPermission() async {
    final accessWhenInUse = await Permission.locationWhenInUse.status;
    switch (accessWhenInUse) {
      case PermissionStatus.denied:
      case PermissionStatus.restricted:
        final permission = await Permission.locationWhenInUse.request();
        if (permission == PermissionStatus.granted) {
          final accessAlways = await Permission.locationAlways.status;
          switch (accessAlways) {
            case PermissionStatus.denied:
            case PermissionStatus.restricted:
              ...
              if (permission == PermissionStatus.granted) {
                ...
                Fluttertoast.showToast(msg: 'Allow the location to be used permanently');
                return false;
              }
